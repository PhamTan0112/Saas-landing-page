    ...
    <section id="pricing" className="py-5">
      <Container className="text-center">
        ...
        <Paragraph className="mt-4 mb-8">
          Flexible, transparent pricing for every business — scale with no
          hidden fees.
        </Paragraph>
      </Container>
      <Container>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {pricingPlans.map((plan, key) => (
            <div key={key} className="relative group h-full ">
              <div className="bg-gradient-to-r from-blue-600 to-violet-600 rounded-3xl h-full">
                <div className="bg-box-bg border border-box-border rounded-3xl shadow-lg shadow-box-shadow p-8 flex flex-col h-full relative">
                  {plan.bestValue && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary text-white text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full bg-gradient-to-t from-blue-600 to-violet-600 ">
                      Best Value
                    </div>
                  )}
